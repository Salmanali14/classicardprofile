...
export default function Defaultprofile({userData,downloadVcf,sendUserData}) {
...
console.log(selectedLink)
  ...
  return (
<>
<div className='w-[100%]  flex items-center flex-col  shadow-2xl min-h-[100vh]' style={!userData?.customTheme  ? {background:`${userData?.backgroundColor}`} : { backgroundImage: `url(${userData.customTheme})`,backgroundRepeat:"no-repeat",objectFit:"cover",backgroundSize:"cover" }} >
<div className=' flex justify-center items-center w-[100%]'>
<div className='w-[150px] h-[150px] rounded-[50%] relative'>
<img style={{width:"150px",height:"150px",borderRadius:"50%",marginTop:"50px",objectFit:"cover"}} src={!userData?.profileUrl ?  "https://img.icons8.com/pastel-glyph/300/user-male-circle.png" :userData?.profileUrl}/>
...
<div className='flex justify-center items-center flex-wrap mt-8 gap-1  w-[80%]'>
  {userData?.links?.map((image, index) => (
    <div onClick={() => sendUserData(userData.id,image.id,"social")}>
    <div  onClick={() => window.open(image.value)} key={index} className=' w-[70px] h-[55px] rounded-[10px] flex justify-center items-center'>
      {image && <img width={40} src={image.image} alt={`icon-${index}`} />}
    </div>
    </div>
  ))}
</div>
            {userData?.custom_links?.map((img, index) => (
              <div className="flex items-center justify-center w-[100%]" onClick={() => sendUserData(userData.id,img.id,"custom")}>
              <div key={index} onClick={() => {
                ...
                <img style={{borderRadius:"10px",width:"120px",height:"75px",objectFit:"cover"}} src={img.image ? img.image : logoblack} alt={`image-${index}`} />
            ...
            {videoUrl && ( 
                ...
                <button 
                onClick={downloadVcf} 
                className='w-[90%] h-[50px] bg-[#18b580] flex justify-center items-center font-[600] mt-10' 
                style={{ backgroundColor:userData?.btnStyle === 1 || userData?.btnStyle === 2 ?  `${userData?.btnBgColor}` :'transparent',border:userData?.btnStyle === 3 || userData?.btnStyle === 4 ?  `1px solid ${userData?.btnBgColor}` :'transparent',color:userData?.btnTextColor ?  `${userData?.btnTextColor}` :'#ffffff' ,
                  borderRadius: 
              ...
              >
                <IoMdContacts className='mr-[5px]'  />Save Contact
              </button>
              </div>
<br></br>
